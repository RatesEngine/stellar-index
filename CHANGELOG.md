...
- **`pkg/client.AssetDetail` was missing 11 documented wire
  ...
  `org_name`, `anchor_asset`, `anchor_asset_type`), and all seven
  F2 fields (`circulating_supply`, `total_supply`, `max_supply`,
  `market_cap_usd`, `fdv_usd`, `supply_basis`,
  `volume_24h_usd`). Go's `encoding/json` silently ignores
  unknown fields by default, so consumers got zero-valued
  structs without warning — the only way to access the missing
  fields was dropping to raw HTTP. This was a real wallet-
  integrator gap (the F2 fields are exactly what asset-detail
  UIs need). Adding the fields is purely additive under SemVer
  (`pkg/client` is `v0.x` pre-release; the SDK contract pins
  backwards-compat from `v1.0.0`). Two new tests pin the
  JSON-decode contract and the `omitempty`-on-nil round-trip
  shape so a future regression fires before shipping.
