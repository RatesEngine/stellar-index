...
> **Companion doc:** [`showcase-site-data-inventory.md`](showcase-site-data-inventory.md)
...
| # | Ticket | AC | Est |
|---|---|---|---|
| 0.1 | Bootstrap `web/showcase/` with Next.js 15 + TS + Tailwind + shadcn | `pnpm install && pnpm dev` runs, `/` returns 200 with "Hello, ratesengine" | S |
...
| 0.4 | Add `web/showcase/` job to CI workflow | Typecheck + lint + build all pass on green branch | S |
| 0.5 | README explaining stack, how to run, how to deploy | Renders on GitHub | S |

**Phase 0 deliverable**: an empty Next.js app at `web/showcase/` that builds + lints + has the API types generated and CI green. Foundation for everything else.
...
- Data inventory (the WHAT): [`showcase-site-data-inventory.md`](showcase-site-data-inventory.md)
