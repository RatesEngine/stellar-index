...
| L4.11 | Public status page at `status.ratesengine.net` — decision: cstate on GitHub Pages (independent of our origin infra; matches "everything reviewable in git" stance; can graduate to Statuspage.io later without changing the customer URL). End-to-end operator runbook in [`docs/operations/status-page-setup.md`](../operations/status-page-setup.md): repo scaffold, components list, GitHub Pages workflow, DNS via Cloudflare, incident-posting workflow, RSS subscription path. | Wk 9 | half-day | L4.4 | launch | infra config (operator runbook ready) | 🟡 |
